...
{
	dataProvider->calculateTime();

	// Date, Time
	//bufferLength += addBufferData(DATE, dataProvider);
	bufferLength += addBufferData(TIME, dataProvider);
	bufferLength += addBufferData(SECOND, dataProvider);
...
unsigned char FrSky::addBufferData(const char id, IFrSkyDataProvider* dataProvider)
{
	...
	switch(id) {
		...
		case FUEL :
		{
			...
			int fuelLevel = dataProvider->getBatteryVoltage();
			frskyBuffer[bufferLength] = header_value;
		...
		/*case D_A2_ID : // NOT WORKING!
		{
			int voltageA2 = 100; //round(dataProvider->getBatteryVoltageFloat() );
			frskyBuffer[bufferLength] = header_value;
			frskyBuffer[bufferLength + 1] = D_A2_ID;
			frskyBuffer[bufferLength + 2] = lsByte(voltageA2);
			frskyBuffer[bufferLength + 3] = msByte(voltageA2);
			return 4;
			break;
		}*/
		case INDVOLT :
		...
		case GPSSPEED :
		{
			...
			break;
/*
			// GPS Ground speed in m/s. Seems  like there is a multi of 1.84 for some reason
			int gpsSpeed = round(dataProvider->getGpsGroundSpeed() / 1.84f);
			frskyBuffer[bufferLength] = header_value;
			frskyBuffer[bufferLength + 1] = GPSSPEED;
		...
		case LONGITUDE :
		{
			...
			float gpsLongitude = dataProvider->getLongitude();
			frskyBuffer[bufferLength] = header_value;
		...
		{
			frskyBuffer[bufferLength] = header_value;
			frskyBuffer[bufferLength + 1] = YEAR;
		...
		case TIME :
		{
			...
			frskyBuffer[bufferLength + 2] = byte(dataProvider->getHour());   // no lsByte / msByte!
			frskyBuffer[bufferLength + 3] = byte(dataProvider->getMinute());
			return 4;
			break;
		}
		case SECOND :
		{
			frskyBuffer[bufferLength] = header_value;
			frskyBuffer[bufferLength + 1] = SECOND;
			frskyBuffer[bufferLength + 2] = lsByte(dataProvider->getSecond());
			frskyBuffer[bufferLength + 3] = msByte(dataProvider->getSecond());
			return 4;
			break;
		}
		case ACCX :
		...
		case VFAS :
		{
			int vfasTemp = dataProvider->getBatteryVoltage();
			frskyBuffer[bufferLength] = header_value;
			frskyBuffer[bufferLength + 1] = VFAS;
			frskyBuffer[bufferLength + 2] = lsByte((int)(vfasTemp));
			frskyBuffer[bufferLength + 3] = msByte((int)(vfasTemp));
			return 4;
			break;
		}
		case VOLTAGE : // NOT USED, VFAS INSTEAD!
		{
			//float batteryVoltage = par->termToDecimal(0) * 0.5238f;
			//float batteryVoltage = 100.0f * 0.5238;
			float batteryVoltage = dataProvider->getBatteryVoltageFloat() * 0.5238f;
			frskyBuffer[bufferLength] = header_value;
...
void FrSky::printValues(HardwareSerial* serialPort, IFrSkyDataProvider* dataProvider)
{
	serialPort->print("Voltage: ");
	serialPort->print(dataProvider->getBatteryVoltageFloat(), 2);
	serialPort->print(" Current: ");
	serialPort->print(dataProvider->getBatteryCurrent(), 2);
	serialPort->print(" Fuel: ");
	serialPort->print(dataProvider->getBatteryVoltage());
	serialPort->print(" Latitude: ");
	serialPort->print(dataProvider->getLatitude(), 6);
	serialPort->print(" Longitude: ");
	serialPort->print(dataProvider->getLongitude(), 6);
